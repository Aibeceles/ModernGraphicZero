 ...
 * @author Aibes
 */
public class GenerateBinaryZero implements Callable<String> {
...
public GenerateBinaryZero(int num, int denom, Semaphore startS,Semaphore s1, Semaphore s2,Semaphore s3, Semaphore s4,EndProduct endCproduct,GBList gbList,ResultListBean rListB) {
    numerator=num;
    denominator=denom;
    startSemap=startS;
    sf1=s1;
...
public String call() throws InterruptedException, Exception {
   ...
   System.out.println("GenerateBinaryZero, Threadid" + threadId + " numerator" + numerator );
    while (!endProduct.getEndCproduct()) {
    setIndex=0; 
    nBinaryList.clear();
    nBinaryList1.clear();
    setSize=numerator;
    //System.out.println("GenerateBinaryZero Before aquire" + startSemap.availablePermits());
    System.out.println("Threadid" + threadId +" sf1 permits " + sf1.availablePermits()+" sf2 permits "+sf2.availablePermits()); //System.out.println("sf2 permits "+sf2.availablePermits());
...
//    System.out.println("GenerateBinary after sf2 increment, Threadid" + threadId  );
 ...
 //  System.out.println("GenerateBinary before sb1 ok to iterate, Threadid" + threadId  );
  ...
  System.out.println("                                    GenerateBinaryZero before return, Threadid" + threadId  );
