...
public GenerateBinaryLastIterate(int num, int denom, Semaphore s1, Semaphore s2,Semaphore s3, Semaphore s4,EndProduct endCproduct,GBList gbList,boolean isMu) {
numerator=num;
denominator=denom;
sf1=s1;
sb1=s2;
...
// System.out.println(nextBigDecimal.toString() + " ");
    ...
    System.out.println("GenerateBinaryLastIterate Before while (setSize>1) Numerator, Denomiator, nextBigDecimal: "+numerator + " " + denominator + " " + nextBigDecimal.toString());
 //   gbList.setGBList(nextBigDecimal);
 //   System.out.println("gbl   ThredID" + threadId + " ReturnList" + returnList.toString());
 //   sb2.release(); 
    while (setSize>1)
    {  //  sf2.acquire();
      System.out.println("GenerateBinaryLastIterate inside while loop  setSize,isMu: " + setSize + "  " + isMu  );  
...
//    System.out.println(nextBigDecimal.toString() + " ");
        ...
        System.out.println("GenerateBinaryLastIterate Inside while (setSize>1) Numerator, Denomiator, nextBigDecimal: "+numerator + " " + denominator + " " + nextBigDecimal.toString());
  //        gbList.setGBList(nextBigDecimal);
  //       System.out.println("gbl inside loop ThredID" + threadId + " ReturnList" + returnList.toString());
         setIndex++; if (isMu) {setSize=1;System.out.println("GenerateBinaryIterate Inside isMu"); } // latest prune test, after first setSize=1  
      }    
