...
public class GenerateBinaryLast implements Callable<String> {
...
public GenerateBinaryLast(int num, int denom, Semaphore s1, Semaphore s2,Semaphore s3, Semaphore s4,EndProduct endCproduct,GBList gbList) {
numerator=num;
denominator=denom;
sf1=s1;
sb1=s2;
...
public String call() throws InterruptedException {
    ...
    long threadId = Thread.currentThread().getId();
      System.out.println("GenerateBinaryLast, Threadid" + threadId + " numerator" + numerator );
  ...
  //       System.out.println("gbl inside loop ThredID" + threadId + " ReturnList" + returnList.toString());
   System.out.println("Threadid" + threadId + " nextBigDecimal " + nextBigDecimal.toString() );
    ...
    endProduct.setEndCproduct(true);
      System.out.println("Threadid" + threadId + "                           endproduct is true !!!!!!  " );
  System.out.println("Before sb2.release Threadid" + threadId +" sb1 permits " + sb1.availablePermits()+" sb2 permits "+sb2.availablePermits());
      sb2.release(); }  // end of while loop
