...
public GenerateBinaryZeroIterate(int num, int denom, Semaphore startS,Semaphore s1, Semaphore s2,Semaphore s3, Semaphore s4,EndProduct endCproduct,GBList gbList,ResultListBean rListB, boolean isMu) {
    numerator=num;
    denominator=denom;
    startSemap=startS;
    sf1=s1;
...
public String call() throws InterruptedException, Exception {
    ...
    System.out.println("generateBinaryZeroIterate before sf2.acquire()0"+"threadId: "+threadId);
    while (!endProduct.getEndCproduct()) {
    setIndex=0; 
    nBinaryList.clear();
    nBinaryList1.clear();
     //setSize=2;             //
    setSize=numerator;    // commented out for iteration prluning 
...
//   System.out.println("GenerateBinary before sf2 increment, Threadid" + threadId  );
    sf2.acquire();
    System.out.println("GenerateBinaryZero after sf2 increment, Threadid" + threadId  );
   ...
   //     returnList.add(nextBigDecimal.toString());
     ...
     System.out.println(" GenerateBinaryZero Iterate Before while(setSize>1) loop Numerator, Denomiator, nextBigDecimal: "+numerator + " " + denominator + " " + nextBigDecimal.toString());
    //    gbList.setGBList(nextBigDecimal);
    }
  //System.out.println(sf1.availablePermits());  System.out.println(sf2.availablePermits());   
    while (setSize>1)
    {
      System.out.println("GenerateBinaryZero inside while loop  setSize,isMu: " + setSize + "  " + isMu  );
   ...
   //      returnList.add(nextBigDecimal.toString());
     ...
     System.out.println("GenerateBinaryZeroIterate Inside while (setSize>1) Numerator, Denomiator, nextBigDecimal: "+numerator + " " + denominator + " " + nextBigDecimal.toString());
  //       gbList.setGBList(nextBigDecimal);
         setIndex++;
        if (isMu) {setSize=1;System.out.println("GenerateBinaryIterate Inside isMu"); } // latest prune test, after first setSize=1
      }    
     testList8.clear();
     testList5.clear();
     nBinaryList.clear();
 ...
 //System.out.println("GenerateBinary before sf1 iterated, Threadid" + threadId  );
    sf1.release();
   System.out.println("GenerateBinaryZero before sb1 ok to iterate, Threadid" + threadId  );
    sb1.acquire();
   System.out.println("GenerateBinaryZero after sb1.aquire before sf2.release, Threadid" + threadId  );
    sf2.release();
   } // endproduct while
